...
LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
  ...
  if (enabled.load()) {
    ...
    case LEFT_CLICK_KEY:
      if (isDown)
        leftClickPressed.store(true);
      if (isUp)
        leftClickPressed.store(false);
      return 1;
    case RIGHT_CLICK_KEY:
      if (isDown)
        rightClickPressed.store(true);
      if (isUp)
...
void physicsLoop() {
  ...
  while (running.load()) {
    ...
    if (enabled.load()) {
      ...
      vx *= 0.6;
      vy *= 0.6;
      // keep px/py synced with current cursor (in case user moved with mouse)
      POINT curp;
